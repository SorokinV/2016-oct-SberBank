...
for (mcc in unique(agg2.mcc.ts$mcc_code)) {
  ...
  zz.ts.pacf   = zz.ts.pacf[zz.ts.pacf>=4]
  ...
  if (zz.ts.xreg) {
    ...
    zz.ff  = fourier(ts(zz.ts,frequency = zz.ts.pacf[1]),K=(zz.ts.pacf[1]-1)%/%2)
    zz.fff = fourier(ts(zz.ts,frequency = zz.ts.pacf[1]),K=(zz.ts.pacf[1]-1)%/%2,h=30)
                                    
    if (length(zz.ts.pacf)>1) {
    for (i in zz.ts.pacf[2:length(zz.ts.pacf)]) {
      
      zz.ff  = cbind(zz.ff, fourier(ts(zz.ts,frequency = i),K=(i-1)%/%2))
      zz.fff = cbind(zz.fff,fourier(ts(zz.ts,frequency = i),K=(i-1)%/%2,h=30))
  ...
  rm(zz.lm.names)
  rm(zz.ts.pacf,zz.ff,zz.fff,zz.ffx)
